...
pub fn mo_cases<'a>(g : &'a EGraph, missing : &'a Vec<(NodeIndex, NodeIndex)>) -> impl Iterator<Item = EGraph> + 'a {
...
pub fn eo_cases<'a>(g : &'a EGraph, missing : &'a Vec<(Argument, Argument)>) -> impl Iterator<Item = EGraph> + 'a {
    flip_iterator(&missing).map(| q | {
        let mut gp = g.clone();
        for (q1, q2) in q {
...
pub fn missing_eo(g : &EGraph) -> Vec<(Argument, Argument)>{
    pair_fmap(&g, |x, y| {
        match (&g[x], &g[y]) {
            (EPair(hdl1, mid1, e1), EPair(hdl2, mid2, e2)) => {
                if hdl1 == hdl2 && mid1 < mid2 && !has_path_connecting(&g.clone(), x, y, None) && !has_path_connecting(&g.clone(), y, x, None){
