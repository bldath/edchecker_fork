...
use petgraph::{
    ...
    visit::{Dfs, EdgeFiltered, EdgeRef, IntoEdgesDirected, IntoNeighbors, NodeRef},
    Direction::Outgoing,
};

use crate::model::EdgeTp::*;
use crate::{
    model::{Argument, EGraph, EPair, EdgeTp, Event},
...
pub fn add_edges<I>(g: &mut EGraph, it: I)
where
        ...
        x < y
            && match (&g[x], &g[y]) {
                (EPair(_, _, Event::Write(var1, val1)), EPair(_, _, Event::Write(var2, val2))) => {
                    var1 == var2
                        && val1 != val2
                        && (!g.contains_edge(x, y) && !g.contains_edge(y, x))
                }
                _ => false,
            }
    })
...
pub fn try_extend(g: &mut EGraph, e: (EdgeTp, NodeIndex, NodeIndex)) {
    ...
    if has_path_connecting(&*g, x, y, None) {
        ...
        println!("Cannot infer direction of edge: {:?}", e)
    }
}
